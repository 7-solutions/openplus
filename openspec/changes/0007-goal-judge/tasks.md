...
- [ ] T-440 RED `TestGoalAbsentSkipsJudge`: existing test `Run`
      continues to behave exactly as today when `Options.Goal` is
      empty and `Options.Judge` is nil — no provider call, no extra
      turns. RED until `Options.Goal` / `Options.Judge` exist.
- [ ] T-441 RED `TestGoalEmptyStopsImmediately`: when `Options.Goal`
...
- [ ] T-442 RED `TestGoalJudgeStopsLoopWhenMet`: a scripted agent
...
- [ ] T-443 RED `TestGoalJudgeKeepsLoopingWhenUnmet`: agent wants to
      ...
      as a user message.
- [ ] T-444 RED `TestGoalJudgeRespectsMaxIterations`: judge always
...
- [ ] T-445 GREEN: extend `runtime.Session` with `Goal string`,
...
- [ ] T-450 RED `TestMainGoalFlag`: subprocess test; `--goal 'foo'`
      reaches `Session.Goal`. Same subprocess pattern as
      `--config` (build binary into temp, exec, assert via
      `--fake -p 'foo'` smoke).
- [ ] T-451 RED `TestMainGoalEnvOverride`: `OPENPLUS_GOAL='foo'`
      wins over `--goal 'bar'` (env > flag precedence).
- [ ] T-452 GREEN: `cmd/openplus/main.go` adds `flag.StringVar(&goal,
...
- [ ] T-460 Flip T-440..T-452 to `[x]` with commit hashes in
      `openspec/changes/0007-goal-judge/tasks.md`.

## Verification (Gate 5 — before declaring 0007 done)
- [ ] `go build ./...` clean.
- [ ] `go test ./...` **22/22 + 7 new tests** green.
- [ ] `go test ./internal/runtime/...` — old + new tests green.
- [ ] `go run ./cmd/openplus --goal 'ship hello' --fake -C $(mktemp -d)
      -p 'ship hello'` exits 0; with no `--goal` it's identical to
      pre-0007 behavior (regression guard).
- [ ] `OPENPLUS_GOAL='foo' go run ./cmd/openplus --goal 'bar' --fake -C
