# Change 0017 — Tasks (COMPLETE)

> One task = one vertical slice. TDD red-first, ports/adapters, cgo-free.
> `[ ]` open · `[~]` in progress · `[x]` done.

## H0 — Decision
- [x] T-1700 Write `docs/adr/0012-color-vision-theme.md`: new accessibility
...
- [x] T-1710 `Palette` struct of semantic roles (PromptColor, WorkingIndicator,
...
- [x] T-1711 `default` palette: colorblind-safe (Okabe–Ito/Wong-derived,
      luminance-distinguishable, no red/green pairs). Red: default palette exists and
      is returned by `Default()`.

## H2 — Contrast gate (accessibility)
- [x] T-1712 WCAG relative-luminance contrast function for two hex colors. Red: known
      pairs return expected ratios (black/white ≈ 21, a bad pair < 4.5).
- [x] T-1713 Contrast test over every shipped palette's text/background pairs asserts
      ≥ 4.5:1. Red: a deliberately-bad pair fails the test; shipped palettes pass.

## H3 — Per-deficiency palettes
- [x] T-1714 `deutan`, `protan`, `tritan` palettes, each tuned + contrast-verified by
      T-1713. Red: each is selectable by name and passes the contrast gate.

## H4 — Model integration
- [x] T-1715 The `internal/tui` model reads colors from the active palette (no raw
